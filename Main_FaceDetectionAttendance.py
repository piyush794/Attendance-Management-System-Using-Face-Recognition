...
def manually_fill():
    ...
    def err_screen_for_subject():
        ...
        Label(ec, text='Please enter your subject name!!!', fg='blue',
    ...
    if l1 == '':
        ...
        try:
            ...
            Notification.configure(
                text=res, bg="royalblue", width=50, font=('calibri', 25, 'bold'))
            Notification.place(x=450, y=130)
...
def subjectchoose():
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    global cursor
                    connection = pymysql.connect(
                        host='localhost', user='root', password='root', db='temp')
                    cursor = connection.cursor()
                except Exception as e:
                    print(e)
    ...
    Notifica = tk.Label(windo, text="Attendance filled Successfully", bg="yellow", fg="black", width=33,
                        height=2, font=('times', 15, 'bold'))

    def Attf():
        import subprocess
        subprocess.Popen(
            r'explorer /select,"Attendance"-------Check attendance-------"')
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'Piyush' and password == 'piyush':
            win.destroy()  # Close the login window after successful login
            ...
            # Define the path to the CSV file (make sure this path is correct)
            cs = "StudentDetails/StudentDetails.csv"

            try:
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0  # Row counter for the grid layout

                    # Loop through each row in the CSV file and display it in the window
                    for row in reader:
                        c = 0  # Column counter for the grid layout
                        for item in row:
                            # Create a label for each item in the row
                            label = tk.Label(root, text=item, relief=tk.RIDGE, width=20, height=2,
                                             font=('times', 12, 'bold'), bg="white")
                            label.grid(row=r, column=c)
                            c += 1
                        r += 1

                # Start the Tkinter main loop for the student details window
                root.mainloop()
            except FileNotFoundError:
                # Handle case when the CSV file is not found
                error_label = tk.Label(root, text="File not found!", fg="red", font=("times", 16, "bold"))
    ...
    # Create a label for showing the error message
    Nt = tk.Label(win, text="Attendance filled Successfully", bg="Green", fg="white", width=40,
                  height=2, font=('times', 19, 'bold'))

    # Username and Password labels
    un = tk.Label(win, text="Enter username : ", width=15, height=2, fg="black", bg="silver",
                  font=('times', 18, ' bold ')).place(x=30, y=50)

    pw = tk.Label(win, text="Enter password : ", width=15, height=2, fg="black", bg="silver",
                  font=('times', 18, ' bold ')).place(x=30, y=150)
    ...
    c0 = tk.Button(win, text="Clear", command=c00, fg="white", bg="black", width=10, height=1,
                   activebackground="white", font=('calibri', 15, ' bold ')).place(x=690, y=55)

    c1 = tk.Button(win, text="Clear", command=c11, fg="white", bg="black", width=10, height=1,
                   activebackground="white", font=('calibri', 15, ' bold ')).place(x=690, y=155)

    # Login button
    Login = tk.Button(win, text="Login", fg="black", bg="SkyBlue1", width=20, height=2, activebackground="Red",
                      command=log_in, font=('times', 15, ' bold ')).place(x=290, y=250)
...
def trainimg():
    ...
    Notification.configure(text=res, bg="royalblue", fg="black", height=2,
                           width=25, font=('calibri', 25, 'bold'))
    Notification.place(x=450, y=110)
...
message = tk.Label(window, text="Attendance Management System using Face Recognition", bg="black", fg="white", width=65,
                   height=2, font=('calibri', 30, ' bold '))

message.place(x=0, y=15)
...
lbl = tk.Label(window, text="Enter Id : ", width=15, height=2,
               fg="black", bg="silver", font=('times', 20, 'bold'))
lbl.place(x=230, y=200)


# this is used for checking of enrollment no it should be only digits
def testVal(inStr, acttyp):
    if acttyp == '1':  # insert
        if not inStr.isdigit():
            return False
...
lbl2 = tk.Label(window, text="Enter Name : ", width=15, fg="black",
                bg="silver", height=2, font=('times', 20, ' bold '))
lbl2.place(x=240, y=300)

txt2 = tk.Entry(window, width=20, bg="white",
                fg="black", font=('times', 25))
txt2.place(x=550, y=310)

clearButton = tk.Button(window, text="Clear", command=clear, fg="black", bg="grey60",
                        width=10, height=1, activebackground="black", font=('calibri', 15, ' bold '))
clearButton.place(x=950, y=210)

clearButton1 = tk.Button(window, text="Clear", command=clear1, fg="black", bg="grey60",
                         width=10, height=1, activebackground="white", font=('calibri', 15, ' bold '))
clearButton1.place(x=950, y=310)

AP = tk.Button(window, text="Admin User", command=admin_panel, fg="black",
               bg="orange", width=19, height=2, activebackground="white", font=('calibri', 15, ' bold '))
AP.place(x=550, y=550)

takeImg = tk.Button(window, text="Take Images", command=take_img, fg="black", bg="white",
                    width=20, height=2, activebackground="orange", font=('calibri', 17, ' bold '))
takeImg.place(x=90, y=430)

trainImg = tk.Button(window, text="Save Images", fg="black", command=trainimg, bg="white",
                     width=20, height=2, activebackground="orange", font=('calibri', 17, ' bold '))
trainImg.place(x=390, y=430)

FA = tk.Button(window, text="Face Attendance", fg="black", command=subjectchoose,
               bg="white", width=20, height=2, activebackground="orange", font=('calibri', 17, ' bold '))
FA.place(x=690, y=430)

quitWindow = tk.Button(window, text="Fill Attendance", command=manually_fill, fg="black",
                       bg="white", width=20, height=2, activebackground="orange", font=('calibri', 17, ' bold '))
quitWindow.place(x=990, y=430)
